...
{
    public function addWork($work_date, $spend, $period_input)
    {
        $work = new Work();
    ...
    public function addPeriodInput(Request $request)
    {
        ...
        $works = Work::whereNotNull('period_input')->orderBy('work_date')->get();
    ...
    {
        $data = $request->all();
        $works = Work::where('work_date', '>=', $data['start_date'])->where('work_date', '<=', $data['end_date'])->orderBy('work_date', 'desc')->get();
